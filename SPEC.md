...
この5値による証拠評価（`result_classification`）は、2.2.1で追記する利用者向け最終回答の構造とは独立した別軸である。証拠評価そのものを弱体化・廃止することはない。

### 2.2.1 証拠状態に関わらず、利用者向け最終回答は一つの結論（神託）を示す

2026-07-18、実際にClaude／Codex／Grok／agyの4 AIで「神は存在しますか？」を検証した際、`result_classification: conflicting`のまま「信仰では存在する、無神論では存在しない、不可知論では知り得ない」と複数の立場を並べるだけで、利用者へ判断を委ねる最終回答が生成された。これはOracle Councilの目的（§1: 「根拠のある回答を返す」）に反するとユーザーが判断し、次の方針を追加した。

「一つの結論を返すこと」と「証拠状態を`conflicting`／`withheld`として正直に記録すること」は矛盾しない。証拠評価（2.2）はそのまま維持し、synthesize・auditフェーズが生成・監査する利用者向け最終回答の**構造**だけを次のとおり定める。

最終回答は、証拠状態とは別に、質問に対する一つの「神託」を先頭へ明記する。許容される神託には次を含む（これらに限定しない）。
...
「判定不能」や「現時点の証拠では確認できない」も、それ自体で完結した正式な結論である。根拠のない「存在する／存在しない」の断定を強制する意図ではない。ただし、複数の立場を並べるだけで結論を利用者へ委ねることは禁止する。

最終回答の構造は次の4要素とする。

1. **神託**: 質問に対する一つの結論を、最初に一文で明記する
2. **理由**: その結論を採用した主要な理由
3. **採用しなかった見解**: 主要な反対意見を示し、最終結論として採用しなかった理由
4. **不確実性**: 残っている不確実性と証拠上の限界

例えば証拠状態が`conflicting`のままでも、次のように回答できる。「神託：現時点で客観的に検証可能な証拠からは、神の存在を確認できません。」——この結論は「神が存在しないと証明された」という意味ではなく、証拠状態は`conflicting`または`unverified`のまま保持し、不確実性欄でその限界を説明する。

auditフェーズは、この4要素の構造が満たされているかを確認する（結論そのものの当否や、特定の結論への誘導は行わない）。構造が欠けている場合は`changes_required`とする。auditは根拠のない断定を要求しない。

実装は`adapters/base.py`の`build_phase_input()`が持つsynthesize・audit向けプロンプト指示（`_ORACLE_VERDICT_GUIDANCE_SYNTHESIZE`・`_ORACLE_VERDICT_GUIDANCE_AUDIT`）に限定し、`AgentResult`のスキーマ（`{"answer": "<string>"}` / `{"status": ..., "issues": [...]}`）は変更していない。

**運用上判明した3つの追加修正（2026-07-18、実際に「神は存在しますか？」を実行して発見）**:

1. **claim_extractの関連性スコープ**: 実行時、AI自身の回答姿勢についての自己言及（例:「AIは断定を避ける」）がclaimとして抽出され、verifyで`contradicted`と判定されて、質問の内容と無関係にStage 1で早期withheldになる事例が見つかった。証拠評価規則（2.2）自体は変更せず、`build_phase_input()`のclaim_extract向け指示へ、質問への回答に無関係なAI自身の振る舞い・文章構成・生成過程についての記述を除外し（ただし質問自体がAIの性質を尋ねる場合を除く）、本質的でないclaimは`minor`重要度とする指示を追加した。
2. **audit指摘のre-synthesizeへの伝播**: 修正サイクルにおいて、再synthesize呼び出しが最初のsynthesizeと全く同じcontext（`question`/`responses`/`claims`/`evidence`/`critique`）しか受け取っておらず、直前のauditが何を指摘したかを一切知らないまま再生成していたことが判明した。`_PHASE_CONTEXT_KEYS["synthesize"]`へ`audit_issues`（直前auditの`issues`、初回は空配列）を追加し、修正1回でauditの具体的指摘へ対応できるようにした。
3. **withheldになった草稿の監査証跡保存**: `--store-content`指定時、synthesizeの草稿とauditの判定・指摘が、withheldになった場合も含めて一切永続化されていなかった（「監査で落ちたが何が落ちたか運営側にも分からない」状態）。`agent_execution_succeeded`イベントへ、synthesize/auditフェーズかつ`--store-content`時のみ`draft_answer`（またはaudit系フィールド）と`"disclosure": "internal_audit_trail_only"`を追加し、監査・デバッグ用途に限定して保存する。利用者向けの公開`final_answer`（`RunResult.final_answer`）とは別経路であり、混同しない。`RunMetadataRecord`へも`audit_status`を追加し、`run_completed`イベントの永続metadataだけで「なぜwithheldになったか」を再構成できるようにした。
...
`verify`のAI呼び出しは通常7回、Clarifierを含め8回、修正を含め10回を基準とする。`quick`のAI呼び出しは通常4回（`respond` 2回、`compare` 1回、`synthesize` 1回）を基準とする。同一Agent retryはRun全体で2回、substitutionは別枠でRun全体1回、全AI呼び出しは`TokenBudget.reserve()`による12回を絶対上限とする。差分スキャン、コンテキスト縮約、Evidence収集はAI呼び出しに数えない決定的なローカル処理とする。
...
Run開始時に利用可能な適格Agent（eligible agents）が5件以上存在する場合、そのRunに参加する構成員（selected participants）を最大4件に制限する。
- **選定手順**: 利用可能と判定されたエージェントを、全体の決定的優先順位（`role_priority` 値の最大値の降順、同順位なら設定順の昇順）でソートし、その先頭最大4件を選出する。選出されたエージェントIDリストを元の設定順に並び戻したものを `selected participants` とし、`ExecutionPlan.participants` に保持する。各フェーズの割り当て計算（`PhaseAssignment` 候補者）の対象は、この `selected participants` に制限される。
- **概念の分離**:
  1. **configured adapters** (多重度 0..*): 設定から読み込まれたエージェント全件。
  2. **eligible agents**: Run開始時にプローブ成功等の条件を満たし、利用可能と判定されたエージェント。
  3. **selected participants** (多重度 2..4): 優先順位に基づいて選出された、そのRunのCouncil構成員。
  4. **executions**: 実際に実行された個別エージェント実行記録。リトライ、代行等により `selected participants` の一部しか実行されない場合がある。
- **participants定義の統一**: `run_created` イベント、永続化される `RunMetadataRecord`、CLI JSONトップレベルの `participants` は、実際に実行されたエージェントのユニーク集合や設定全件ではなく、この `selected participants` に統一される。
...
`ProbeResult`は`status`（OK, COMMAND_NOT_FOUND, QUOTA_EXCEEDED などの状態文字列）と `capabilities`（`AgentCapabilities`、プローブ失敗時は None でもよい）を持つ。
...
`AgentResult`は`status`、`structured_output`、`raw_output_hash`、`usage`、`process_exit_code`、`started_at`、`finished_at`、`error_code`、`error_summary`を持つ。入力はUTF-8のstdinまたはAdapter管理の一時ファイルを使う。stdoutは結果、stderrは診断専用とし、Orchestratorへ返す前にschema検証とsecret redactionを行う。
...
### 10.4.2 Claim性質（claim_nature、X-9）

`claim_nature`は、Claimがどんな種類の主張かを`importance`・`claim_role`・`status`とは独立に表すEnumとする。省略時は後方互換のため`factual`とする。

- `factual`: 外部Evidenceで確認・反証しうる、世界についての主張
- `reasoning`: 他の前提・Claimからの論理的推論
- `opinion`: 主観的な意見・好み
- `normative`: 価値判断、推奨、「〜すべき」「〜は個人が決めることである」等の当為の主張
- `hedge`: 不確実性・未決着であることそのものについての言明
- `structural`: 回答の構成に関する足場的表現で、実質的な主張ではないもの

Claim Extractorが`claim_nature`を提案する。同じ種類の主張は言い回しが異なっても同じ`claim_nature`になるべきで、文言の一致に依存した扱いをしてはならない。

`opinion` / `normative` / `hedge` / `structural`は、10.5の`not_applicable`（意見、提案、創作など事実検証の対象外）に該当する。VerifierはこれらのClaimを`unverified`ではなく`not_applicable`とするよう指示されるが、Agentが指示に従わず`unverified`を返した場合は、Orchestratorが決定的に`status`を`not_applicable`へ正規化する（`contradicted`・`conflicting`は正規化しない。Agentがより具体的な問題を指摘したとみなし、10.5・11章の既存規則をそのまま適用する）。この正規化は`factual`・`reasoning`のClaimには適用されない。critical/majorな`factual`Claimが未検証の場合に安全側（`withheld`）へ倒す既存の判定は変更しない。

evidence_collect（§10.2）でも、`opinion` / `normative` / `hedge` / `structural`のClaimは検索・取得の対象から外す（事実検証の対象外であるものに検索予算を使わないため）。既存の対象選定（critical/majorの上位5件）はそのまま維持し、この除外はその前段で適用する。
...
- `unverified`: 確認できない
- `not_applicable`: 意見、提案、創作など事実検証の対象外（§10.4.2の`claim_nature`が`opinion`/`normative`/`hedge`/`structural`のClaimはこれに該当する）
...
- 質問整理 (対話モードのみ)
- 独立回答 (`respond`フェーズ、2 Agent)
- 簡易比較 (`compare`フェーズ、1 Agent)
- 統合回答 (`synthesize`フェーズ、1 Agent)
- 外部Evidence収集なし

`quick` モードは、外部Evidence収集・Claim抽出・Claim検証・最終監査を省略し、簡易比較と統合回答のみを素早く行う。
実行フェーズの順序は `respond` (スロット0) -> `respond` (スロット1) -> `compare` -> `synthesize` となり、計4回のエージェント呼び出しが行われる。
監査（`audit`）を行わないため、`synthesize`フェーズに対する auditor との分離制約は適用しない。
外部検証を行わないため、結果の最終分類 (`ResultClassification`) は常に `ResultClassification.UNVERIFIED` ("unverified") となり、`oracle_exit_code = 0` (成功) で完了する。回答保留 (`withheld`) は発生しない。
出力へ`external_verification: false`を必ず含める。
また、CLI JSONやイベント、RunMetadataRecord に `mode` フィールドを保持し、進捗表示も `quick` に適合した4ステップ形式にする。
...
### 13.1.1 デバッグ用トレース表示（--trace / --trace-output、X-9）

通常実行では、各AgentのPhase生出力（raw response）はCLI JSON・Storage・履歴表示のいずれにも一切出力しない（§11.5・§15.8）。デバッグ目的で明示指定した場合だけ、Phaseごと・Agentごとの生出力を確認できる。

```bash
oracle ask "質問" --trace
oracle ask "質問" --trace-output trace.json
```

- `--trace`: 各Phase・各Agentの生出力（best-effort redaction適用済み）をstderrへ表示する
- `--trace-output PATH`: 同内容をJSONファイルへ書き込む（`--trace`併用時はstderrにも表示する）
- 出力項目は最低限、phase名、agent名、attempt番号、開始・終了時刻（elapsed_ms）、成功・失敗状態、process exit code、parse後の構造化出力、redaction適用有無を含む
- このトレース経路はStorage Contract（`--store-content` / `--no-store`）と完全に独立している。メモリ上にのみ保持し、`--trace-output`を明示指定しない限りディスクへ書き込まない。`--no-store`と併用しても矛盾しない
- redactionはbest-effort（APIキー・Bearerトークン・ホームディレクトリパスらしき文字列等をパターンで除去）であり、完全性を保証しない
...
- 子AI CLIの終了コードは`AgentResult.process_exit_code`／`AgentExecution.process_exit_code`として記録し、Oracle Council自身のoracleExitCode（実装フィールド名`oracle_exit_code`）と混在させない（S-8）
...
トップレベルの正式フィールドは`oracle_exit_code`とする（S-8）。既存利用者のため、schema version 1.xではトップレベル`exit_code`を互換エイリアスとして残し、全経路で必ず`oracle_exit_code`と同値にする。`executions[]`の各要素には子CLI processの`process_exit_code`を出力し（取得不能時は`null`）、`executions[]`へ曖昧な`exit_code`は出力しない。この追加だけを理由に`schema_version`を2.0へ上げない。

Run生成前に停止する場合も、CLIは停止理由を構造化結果として必ず返す。`run_id`は`null`とし、`status`と`oracle_exit_code`（互換エイリアス`exit_code`同値）を正本にする。
...
- EvidenceErrorCode: `SEARCH_UNAVAILABLE` / `ALL_FETCH_BLOCKED` / `EVIDENCE_TIMEOUT` / `BUDGET_EXHAUSTED` / `FETCH_FAILED` / `INVALID_URL_ENCODING`

`evidence_collect.success_count`はEvidence件数ではなく、収集処理が正常終了した回数を表す。正常終了時はEvidence 0件でも`1`、検索経路全断などPhase失敗時は`0`とする。Evidence件数、検索回数、fetch回数、fetch失敗コード別件数は`Phase.metrics`へ記録する。

`Phase.metrics`には、Phase.status/EvidenceOutcomeとは別の第三の軸として、`provider_type`（`fake` / `manual` / `cli_search`）と`real_search_performed`（真偽値）を記録する（X-9）。`FakeEvidenceProvider`（既定）と`ManualEvidenceProvider`（`--evidence-file`）は実検索を行わないため`real_search_performed=false`固定であり、`search_count`は常に`0`のままとなる。「Fake Providerが固定の1件（実質空）のEvidenceを返して処理が成功した」ことと、「実検索を行ったが根拠が見つからなかった」（`real_search_performed=true`かつ`evidence_count=0`）は明確に異なる状態であり、Phase.status（succeeded）だけを見て両者を混同してはならない。既定のFake Evidenceのままでは、事実確認を要するcritical/majorのClaimが根拠不足で`unverified`になりやすく、結果として回答が保留（`withheld`）になりやすい。この既定挙動自体は後方互換のため変更しないが、CLIは`--adapter-mode real`かつEvidence Provider省略時にこの事実をstderrへ警告する（`--json`時は表示しない）。
...
`EvidenceErrorCode.INVALID_URL_ENCODING`は、検索結果URL/IRIをHTTPリクエスト用URIへ安全に変換できない場合に使用する。個別候補URLの変換失敗はRun全体を失敗させず、EvidenceFetchErrorとしてfetch失敗件数へ集計して次候補へ進む。

検索が正常終了して資料が見つからなかった場合は`succeeded`＋`no_evidence`＋Claim `unverified`である。「根拠が見つからなかった」と「Evidence機能が壊れて調べられなかった」を分けて記録・表示する。

Run開始後の`SearchError`は`evidence_collect` Phaseを`failed`にし、`success_count=0`、`error_code=<SearchError.code>`、`metrics.search_error_codes`へコード別件数を記録してRunを`failed`にする。CLI JSONの外部statusは後方互換のため`verification_unavailable`、exit codeは`3`とし、生stdout/stderr、検索prompt、環境変数、例外全文は出力しない。

Ctrl+Cやキャンセル要求が発生した際、Orchestratorは実行中の各AgentExecutionの所有関係をスレッドセーフに管理するExecutionRegistryを用いてアクティブなExecutionを特定し、各Adapterへ並行してキャンセルを伝播する。各Adapterは子process treeをterminateし、5秒後も残る場合はkillする。進行中のExecution、Phase、Runを`cancelled`として保存する。
...
#### RunMetadataRecord（既定で永続化する唯一 of Runレコード）
...
- `participants`（選定された selected participants エージェントIDリスト）
...
- `metrics`（metadata区分。`evidence_collect`では検索回数、候補URL件数、fetch試行/成功/失敗件数、Evidence件数、対象Claim件数、Evidence取得済みClaim件数、SearchError/EvidenceFetchErrorのコード別件数を保持する。URL、本文、excerpt、検索語、prompt、stdout/stderr、環境変数は含めない）
...
- `finished_at`
- `elapsed_ms`
- `process_exit_code`（子CLI processのOS終了コード。取得不能・Fake Agentはnull。S-8）
- `response`
- `error_code`
- `error_summary`
- `raw_diagnostic`（content区分）
...
`retry_of`と`substitute_for`は排他的であり、retry/substitutionごとに新しいExecutionと新しいBudgetReservationを作る。失敗Executionを再利用しない。substitution選定時は`agent_substitute_selected`、候補なし時は`agent_substitution_unavailable`をmetadata eventへ記録する。`agent_execution_succeeded`／`agent_execution_failed` eventは`process_exit_code`フィールドを常に持ち、取得不能時は`null`とする（S-8）。eventにはraw診断、prompt、質問、回答、環境変数を含めない。
