...
中立検索で`verified`相当になっても、反証検索を1回実行してから確定する。Claimは`verified`または`contradicted`確定、2クエリ消費、fetch成功3件、Run上限、90秒のいずれかで停止する。上限到達時は`unverified`とし、`EVIDENCE_BUDGET_EXHAUSTED`を記録する。`critical`が`unverified`なら回答を`withheld`とする。
...
- Phase.status: 検索・取得処理そのものの成否
  ...
  - `degraded`: 一部ClaimがRun上限・時間切れで未処理。未処理Claimは`unverified`とし`BUDGET_EXHAUSTED`を記録して続行する
