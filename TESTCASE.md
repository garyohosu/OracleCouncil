...
| 期待する終了コード | CLI境界以外は`N/A`。CLI境界で未確定なら`BLOCKED: QandA R-1` |
| 期待するstdout | CLI境界以外は空。CLIの`--json`以外はケース記載の人間可読出力 |
| 期待するstderr | 成功時は空または進捗、失敗時はredaction済み診断。`--json`時は`BLOCKED: QandA R-2` |
| 期待する保存イベント | Storageを通らないUT/CTは`0件`。Runを作るケースは省略不可または`BLOCKED: QandA S-2/S-3/T-4` |
...
| `verify` | 通常7回、Responder 2並列、Evidence、Critic 1、Synthesizer 1、別Agent Auditor 1 | Evidence Phase状態はM-4 |
| `strict` | 一次/高品質資料必須、未確認主要Claimを除外、根拠不足は保留、高リスク時に確認 | 正常終了の詳細schemaはL-5、終了コードはR-1 |
...
- **期待する状態遷移/呼び出し数**: N/A
- **期待する終了コード/stdout**: `2` (needs_clarification) / 停止理由および不足質問リスト。
- **期待する保存イベント**: N/A
- **未確定仕様への依存**: `BLOCKED: QandA R-1`
...
- **期待結果**: `StorageBackend.append` などの書き込みメソッドが一切呼び出されないこと、または一時フォルダが消去されること。
- **期待する終了コード**: `0`
- **未確定仕様への依存**: なし
...
- **期待する終了コード**: `BLOCKED`
- **未確定仕様への依存**: `BLOCKED: QandA R-1, S-6, T-2`
...
- **期待結果**: 失敗理由に応じた一意な終了コードが正しく返されること。
- **未確定仕様への依存**: `BLOCKED: QandA R-1`
...
- **期待するAgent呼び出し回数**: 7回
- **期待する保存イベント**: `run_created`, `phase_started`, `agent_execution_started`, `agent_execution_succeeded`, `phase_succeeded`, `run_completed` が sequence 順に記録されること。
- **未確定仕様への依存**: `BLOCKED: QandA M-4, S-2`
...
- **期待結果**: OrchestratorがCritical Issueありと判断し、監査ステータスを `changes_required` (または `blocked`) に導出すること。
- **未確定仕様への依存**: なし
...
- **入力**: 3回目の質問やり取り要求。
- **期待結果**: 3ラウンド目の対話を開始せず、2ラウンド完了時点で仮定を確定して進めるか、または終了コード2で停止すること。
...
- **期待結果**: `risk_level` が `strict_required` (または高リスクフラグ) と判定されること。
...
- **期待結果**: 自動仮定を行わず、`strict_required` のステータスで処理を停止すること。
- **未確定仕様への依存**: `BLOCKED: QandA R-1`
...
- **期待結果**: 対象プロセスおよびその子孫プロセスが `SIGTERM` (5秒後 `SIGKILL`) で確実に終了されること。
...
- **期待結果**: 本文とメタデータを正しく含む `EvidenceDocument` が返されること。
- **未確定仕様への依存**: `BLOCKED: QandA S-1`
...
- **関連仕様・UC・SEQ**: SPEC §10.2 / UC: 1
- **前提条件**: テスト用固定データ（または標準入力対話）が機能すること。
- **期待結果**: 定義されたスタブデータ、または入力されたURLを正しく `SearchResult` としてラップして返すこと。
...
- **期待結果**: 1 Run全体のfetch総数が12文書、展開後本文24MB、Evidence総数10件を超えないように検索ループが途中で打ち切られること。
- **未確定仕様への依存**: `BLOCKED: QandA M-4, K-7`
...
- **期待結果**: fetch失敗を構造化結果として上位へ返すこと。Phase/Runを継続するかはM-4確定後にassertする。
- **未確定仕様への依存**: `BLOCKED: QandA S-1, M-4`
...
- **期待結果**: 成功1件と失敗2件を区別して集約できること。継続可否とPhase状態はM-4確定後にassertする。
- **未確定仕様への依存**: `BLOCKED: QandA M-4`
...
- **入力**: `https://127.0.0.2/`
- **期待結果**: ループバック範囲のIPとして検知され、接続が拒否されること。

#### **UT-SHF-03: RFC1918 (プライベートIP) 拒否**
- **テストレベル**: UT
- **対象クラス/機能**: `SafeHttpFetcher` / `safe_fetcher.py`
- **関連仕様・UC・SEQ**: SPEC §16.2
- **入力**: `https://192.168.1.1/config`、`https://10.0.0.5/`
...
- **対象クラス/機能**: `SafeHttpFetcher` / `safe_fetcher.py`
- **関連仕様・UC・SEQ**: SPEC §16.2
- **入力**: 各種クラウドプラットフォームのメタデータURL
- **期待結果**: link-local判定またはカスタムホストチェックで拒否されること。
...
- **期待結果**: `IOError` 等が適切にキャッチされ、プロセス全体がクラッシュせず適切に制御されて上位層へエラーが返されること。
...
- **期待結果**: `--no-store` 指定時、実行中のイベント書き込みは一時領域等に行われるか、または全く行われず、実行終了時にファイルが一切残らないこと。
...
- **対象クラス/機能**: `Orchestrator` / `orchestrator.py`
- **関連仕様・UC・SEQ**: SPEC §8.3
- **期待結果**: 予算オーバー時の `reserve` 失敗により、Orchestratorが `BUDGET_EXCEEDED` エラーで安全に終了すること。
...
- **期待結果**: 失敗したExecutionが消費した実トークン数（またはゼロ）と、再試行側の予約・消費トークン数が重複せず、それぞれ正しく累計に加算されること。
...
- **期待結果**: APIのレスポンス形式がパース可能であり、SearchResultに必要なURL・タイトルが含まれていること。
- **実行制限**: 手動またはNightlyビルド専用（Opt-in）。
- **未確定仕様への依存**: `BLOCKED: QandA S-1`
...
- **期待結果**: `SearchResult`/`EvidenceDocument`型、limit、利用不能の構造化結果がProvider固有実装から漏れない。
- **期待する状態遷移**: N/A
- **期待するAgent呼び出し回数**: 0
- **期待する終了コード/stdout/stderr**: N/A / 空 / 空
- **期待する保存イベント**: 0件
...
- **優先度**: P0
- **自動化可否**: CIで可
- **未確定仕様への依存**: `BLOCKED: QandA S-1, M-4, K-2`
...
- **優先度**: P0
- **自動化可否**: CIで可
- **未確定仕様への依存**: `BLOCKED: QandA L-5, S-2, S-3`
...
- **優先度**: P0
- **自動化可否**: 仕様確定後CIで可
- **未確定仕様への依存**: `BLOCKED: QandA R-1, R-2, S-8`
...
- **期待する終了コード**: `BLOCKED`
- **未確定仕様への依存**: `BLOCKED: QandA J-3, R-1`
...
- **入力**: 質問: `"この薬の服用量は？"`
- **期待結果**: 自動仮定せず、`strict_required` のステータスで処理を即座に停止すること。
- **期待する終了コード**: 非ゼロの特定エラーコード
- **未確定仕様への依存**: `BLOCKED: QandA R-1`
...
- **期待結果**: `verification_unavailable` で処理を停止し、暗黙のうちに `quick` モードへフォールバックしないこと。
- **期待する終了コード**: 非ゼロの特定エラーコード
- **未確定仕様への依存**: `BLOCKED: QandA R-1`
...
- **期待する終了コード**: 非ゼロ
- **未確定仕様への依存**: `BLOCKED: QandA R-1`（Run状態は確定、oracleExitCodeのみ未確定）
...
- **期待結果**: 独立回答および相互監査が不能なため、実行を開始せず（または開始直後に判定し）回答不能としてエラー終了すること。
- **期待する終了コード**: `BLOCKED`
- **未確定仕様への依存**: `BLOCKED: QandA R-1`（1 Agent時の自己監査は廃止済み）
...
- **期待結果**: contradicted Claimを最終回答へ採用せず、回答公開条件を満たさないこと。Run分類・状態・終了コードは確定前にassertしない。
- **期待する終了コード**: `BLOCKED`
- **未確定仕様への依存**: `BLOCKED: QandA T-5, R-1, S-2`
...
- **期待結果**: conflicting Claimを複数説として表示する。Run全体の`result_classification`と終了コードは確定前にassertしない。
- **期待する終了コード**: `BLOCKED`
- **未確定仕様への依存**: `BLOCKED: QandA T-5, R-1`
...
- **期待結果**: majorは削除または未確認表示、criticalは`withheld`とする。Run全体分類は確定前にassertしない。
- **未確定仕様への依存**: `BLOCKED: QandA T-5`
...
- **期待結果**: 予算を消費する新しい呼び出しが発生する直前で `TokenBudget.reserve()` が失敗し、安全に処理が中断され、Run.status が `failed` になること。
- **期待する終了コード**: 非ゼロ
...
- **期待結果**: 13回目の実行に進もうとした時点でOrchestratorが停止し、Runを `failed` として記録すること。
- **期待する終了コード**: 非ゼロ
- **未確定仕様への依存**: `BLOCKED: QandA M-5`
...
- **期待結果**: 起動されたすべてのCLI子プロセスがOSレベルでシグナル（SIGTERM/SIGKILL）によりクリーンアップされ、残留プロセスが0件であること。イベントログに `cancelled` 状態が記録されること。
- **期待する終了コード**: 非ゼロ
- **未確定仕様への依存**: `BLOCKED: QandA T-2`
...
- **関連仕様・UC・SEQ**: SPEC §16.2 / UC: 2
- **前提条件**: 検索結果等から `http://127.0.0.1/` や `http://169.254.169.254/` などの不正URLが渡されること。
- **期待結果**: `SafeHttpFetcher` 内で確実に接続要求が遮断され、外部への不正なHTTPリクエストが発生しないこと。
- **期待する終了コード**: 非ゼロ (または警告付きでのスキップ完了)
- **未確定仕様への依存**: なし
...
- **期待結果**: それまでに書き込まれたJSONLイベントは正常に残り、再起動して `history` コマンド等で読み込んだ際に、破損データにならずに「クラッシュ終了」として状態を部分復元できること。
- **未確定仕様への依存**: なし
...
- **期待結果**: S-3/M-3で確定した破損方針に従い、警告・部分復元またはfail closedの一方だけを返すこと。
- **未確定仕様への依存**: `BLOCKED: QandA S-3, M-3`
...
- **未確定仕様への依存**: `BLOCKED: QandA L-5, S-1`
...
- **期待する保存イベント**: cancellation eventは`BLOCKED: S-2/S-3/T-4`
...
- **優先度**: P0
- **自動化可否**: CIで可
- **未確定仕様への依存**: `BLOCKED: QandA O-2, R-1, T-4`
...
- QandA回答とSPEC反映の両方が完了したcommitでのみBLOCKEDを解除する。
- `quick`、CLI exit code、Evidence Phase、Storage障害、cancel、DNS pinning、Run全体分類は、対応ID確定前にrelease gateへ含めない。
