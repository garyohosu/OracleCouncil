...
**終了コードの暫定override**: R-1が未回答のため、SPECで唯一確定している`needs_clarification = 2`を除き、ケース本文に残る`0`や`非ゼロ`はシナリオ説明であってassert値ではない。すべてのOracle CLI境界ケースの有効な「期待する終了コード」は`BLOCKED: QandA R-1`とする。子CLIの`processExitCode`はS-8確定までOracle終了コードへ流用しない。
...
- **期待結果**: Orchestratorが未解決Critical Issueありと導出すること。`changes_required`と`blocked`の選択はassertしない。
- **未確定仕様への依存**: `BLOCKED: QandA S-2`
...
- **対象クラス/機能**: `Orchestrator` / `orchestrator.py`
- **関連仕様・UC・SEQ**: SPEC §8.3
- **期待結果**: `BUDGET_EXCEEDED` 検出後に追加のAgent呼び出しを開始しないこと。`Run.status`、CLI終了コード、フォールバック有無はassertしない。
- **未確定仕様への依存**: `BLOCKED: QandA T-1, R-1`
...
- **期待結果**: AgentExecutionを1件も開始せず、回答本文を公開せず、Runを`failed`にすること。
- **期待する終了コード**: `BLOCKED`
- **未確定仕様への依存**: `BLOCKED: QandA R-1`（参加可能Agent 1件は回答不能）
...
- **期待結果**: majorを未修飾の事実として断定しないこと（削除または明示的な未確認表示は許容）。criticalは`withheld`とする。Run全体分類はassertしない。
...
- **期待結果**: 予算不足のExecutionを開始せず、Agent呼び出し回数が増えず、`BUDGET_EXCEEDED`を導出すること。Run状態はassertしない。
- **期待する終了コード**: 非ゼロ
- **未確定仕様への依存**: `BLOCKED: QandA T-1, R-1`
...
- **期待結果**: 完了/開始済みAI呼び出し12回の状態で13回目を開始せず、呼び出しカウンタが12のままであること。Run状態はassertしない。
- **期待する終了コード**: 非ゼロ
- **未確定仕様への依存**: `BLOCKED: QandA M-5, T-1, R-1`
...
- **期待結果**: 起動した全CLI process treeがOS適合のterminate/killで終了し、残留processが0件であること。cancelledイベント内容はS-2/S-3確定後にassertする。
- **期待する終了コード**: 非ゼロ
- **未確定仕様への依存**: `BLOCKED: QandA S-6, T-2, R-1`
...
- **期待結果**: `SafeHttpFetcher`でsocket接続0回のまま拒否理由を返すこと。単一候補拒否後のRun継続可否はassertしない。
- **期待する終了コード**: `BLOCKED`
- **未確定仕様への依存**: `BLOCKED: QandA S-1, M-4, R-1`
