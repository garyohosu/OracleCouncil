...
### S-4.1. `ClarificationEngine.inspect()`の責務が資料間で矛盾している

**重要度**: Critical
**箇所**: `CLASS.md` §1（`ClarificationEngine.inspect(question, context)`）/ `TESTCASE.md` §2.3 UT-CE-01〜07 / `QandA.md` S-4
**疑問**: 2026-07-16のS-4再着手に向けた調査で、`ClarificationEngine.inspect()`の入力と責務について資料間の矛盾が判明した。`TESTCASE.md`のUT-CE-01〜07は、`inspect(question)`が質問文（例:「2025年の日本の消費税率は何パーセントですか？」）から直接`ready`/`ready_with_assumptions`/`needs_clarification`/`premise_issue`/`unsupported`/`safety_blocked`を判定する設計を前提にしている（`context`引数への言及がない）。一方`CLASS.md`は`inspect(question, context)`とシグネチャに`context`を明記し、S-4のAUTO_DECIDED回答は「エージェントが返却した構造化結果（JSON出力）の検証」を`ClarificationEngine`の責務としている。UT-CE-01〜07にはこの矛盾を示す「未確定仕様への依存」の記載がない。
**実装への影響**: `inspect()`が質問文から直接判定するのか、Agent出力を検証するだけなのかで、メソッドの実装・呼び出し元・単体テストの設計が根本的に変わる。両方を担う設計であれば、処理段階（決定的判定とAgent出力検証）をどう分離するかも決める必要がある。
**テストへの影響**: UT-CE-01〜07を現状のまま自動化してよいか、`context`を前提に書き換えるべきかが決まらない。
**回答**: 確定（2026-07-18、ユーザー決定）。ClarificationEngineは2段階の責務を持つ。(1) `inspect(question, context=None)` は質問文と既知contextを決定的規則（SPEC Sec7.5 第1・第2段階、QandA S-4.2）で事前判定し、Agent不要と判断できれば最終`ClarificationResult`を返し、critical ambiguityが残る場合はAgentが必要である旨を明示した`ClarificationPreCheck`を返す。(2) `evaluate_agent_output(question, context, output)` はClarifier Agentが返した構造化出力をclarify phase schemaで検証し、critical questionの格上げ等SPEC Sec7.2・Sec7.5の決定規則を適用して最終`ClarificationResult`を返す。両メソッドは別々の公開APIとして分離し、単一の`inspect()`に両責務を混在させない。`CLASS.md`・`TESTCASE.md`をこの決定に合わせて更新済み（本節末尾参照）。

---

### S-4.2. SPEC §7.5 第1段階「決定的既定値」と第2段階「テンプレート規則」が具体化されていない

**重要度**: Critical
**箇所**: `SPEC.md` §7.5 / `FIX_PLAN.md`（該当タスクIDなし）
**疑問**: SPEC §7.5は非対話モードの仮定生成を「1. 地域・通貨・日時などの決定的な規定値 → 2. 質問種別ごとのテンプレート規則 → 3. Clarifier Agentによる構造化された仮定案」の3段階としているが、2026-07-16時点の調査で、第1段階・第2段階についてタスクID・具体的な規則・入力・出力・適用順序がFIX_PLAN.md/QandA.mdのどこにも定義されていないことが判明した（S-4・J-4はいずれも第3段階＝Clarifier Agent呼び出し自体の扱いのみを対象とする）。
**実装への影響**: 第1・第2段階を実装せずに第3段階だけを実装すると、SPECが定める3段階のフォールバック構造が成立しない。
**テストへの影響**: 決定的既定値・テンプレート規則それぞれの単体テストが設計できない。
**回答**: 確定（2026-07-18、ユーザー決定）。第1段階（決定的既定値）は、出力形式・対象読者・長さ・言語・タイムゾーン・時点（時事性のある質問は現在、なければ指定なし）の未指定分をassumptionsへ記録するだけの処理とし、これら単独ではClarifier Agentを呼ばない。第2段階（テンプレート規則）は、要約・説明・比較・一覧/ランキング・コード作成・校正・調査の7カテゴリを対象とし、該当テンプレートで一意に処理できればAgentを呼ばない。実装は`clarification.py`の`default_assumptions()`（第1段階）・`match_template()`（第2段階、現状は判定結果を記録するのみで分岐処理には未使用）として`ClarificationEngine.inspect()`内に実装した。

---

### S-4.3. Clarifier Agentを呼び出す条件・判定主体・通知方法が未定義

**重要度**: Critical
**箇所**: `Orchestrator` / `ClarificationEngine` / `SPEC.md` §6.3、§7.5 / `QandA.md` S-4
**疑問**: `SPEC.md` §6.3は「`verify`のAI呼び出しは通常7回、Clarifierを含め8回」とし、Clarifier Agent呼び出しが条件付き（通常は呼ばれない）であることを示している。また`tests/unit/test_orchestrator.py`には`assert result.call_count == 7`という、Clarifier不使用時の通常フローを前提にした既存テストが複数存在する（少なくとも4箇所）。しかしS-4のAUTO_DECIDED回答は「Orchestratorは...エージェントを決定的に選出し、AgentRequestを実行する」とのみ記述しており、呼び出しを条件付きにする分岐や、その判定条件・判定主体・判定結果の表現について一切触れていない。S-4.2（決定的既定値・テンプレート規則）で解決できなかった場合にのみClarifier Agentを呼ぶ、という大枠はSPECの文言から読み取れるが、その判定をどのコンポーネントが・どのような入力・戻り値で行うかは未定義。
**実装への影響**: 判定主体・戻り値を決めずに実装すると、Clarifier Agentを常時呼び出す実装（`call_count`が常に8になり既存の`call_count == 7`系テストを破壊する）か、判定条件を独自に発明する実装のいずれかになってしまう。
**テストへの影響**: 「通常7回・Clarifier使用時8回」の両経路を回帰確認するテストが設計できない。
**回答**: 確定（2026-07-18、ユーザー決定）。判定主体は`ClarificationEngine`。第1・第2段階を適用してもなお回答や処理結果を大きく変える「critical ambiguity」が残る場合だけClarifier Agentを呼ぶ。critical ambiguityは次の6種類に限定する: 指示対象が特定できない、実行すべき操作が複数に解釈できる、必須の対象・範囲・入力データが欠けている、指示同士が矛盾している、取り返しのつきにくい操作の対象または範囲が不明、安全性・権限・法令順守に関わる重要条件が不明。出力形式・文章量・読者・語調などの不足はcritical ambiguityにしない。`inspect()`の事前判定結果（`ClarificationPreCheck`）は、Agent不要で確定したか、Agentが必要か、assumptions、検出したambiguities、provisional statusを型として保持する。Agentが必要な場合、Orchestratorがclarifyのrole_priority最高の適格Agentを決定的に1体選び、clarify phaseのAgentRequestを最大1回実行する（実装は`Orchestrator._run_clarification()`、対象リポジトリ相対パス`src/oracle_council/orchestrator.py`）。Agent出力検証後、critical questionが残れば`needs_clarification`、非criticalな仮定だけなら`ready_with_assumptions`、問題なければ`ready`、前提誤りは`premise_issue`、対応不能は`unsupported`、安全上停止は`safety_blocked`とする。Agent呼び出し失敗（タイムアウト・不正JSON・schema不一致・空応答等）は既存の失敗分類に従い、`AUTH_REQUIRED`は`auth_required`（exit 3）、それ以外はすべて新規ステータス`clarification_unavailable`（exit 3、SPEC Sec13.4の既存6値のうちexit 3バケットへ割り当て）として処理する。いずれの停止statusでもRunは生成しない（QandA V-1のinsufficient_agentsと同じ事前停止契約）。

---

### S-4.4. Clarifier Agent使用時のAI呼び出し総数変化とCLI進捗表示の扱い

**重要度**: Major
**箇所**: `cli.py`（`sys.stderr.write("[1/7] 質問を整理しています\n")`）/ `SPEC.md` §6.3
**疑問**: `cli.py`の`cmd_ask`は、`verify`モード開始時に固定文字列`"[1/7] 質問を整理しています"`を1回だけstderrへ出力する（実際のフェーズ進捗と連動しない静的な文言で、`test_cli_ask_happy_path`はこの文字列が含まれることだけを検査している）。Clarifier Agentが実際に呼ばれた場合、AI呼び出し総数は7から8へ増える（SPEC §6.3）が、この進捗表示を動的に`[1/8]`へ切り替えるのか、既存の固定文字列`[1/7]`をそのまま維持するのか（維持する場合、Clarifier使用時に表示上の呼び出し数と実際の呼び出し数が食い違う）は決まっていない。
**実装への影響**: 進捗表示ロジックの設計と、既存の`test_cli_ask_happy_path`との整合が変わる。
**テストへの影響**: Clarifier使用時のCLI進捗表示テストが設計できない。
**回答**: 確定（2026-07-18、ユーザー決定）。CLIは`orchestrator.run_verify()`と同じ決定的な`ClarificationEngine().inspect(question)`を呼び出し方式に着手する前に実行し、`agent_required`の値から総数を計算する（ハードコードしない）。Clarifier Agentが不要と判定されれば従来どおり`[1/7]`を維持し、必要と判定された場合だけ`[1/8]`へ切り替える。`cli.py`の`clarify_trigger`マジック文字列分岐（テストからも参照されていない死んだコードだった）は削除し、新しい`--clarify`フラグは追加しない。通常の質問文から`ClarificationEngine`が決定的に判定する経路だけが実装され、実際に到達可能である。
