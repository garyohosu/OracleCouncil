# OracleCouncil 自動継続開発ランナー仕様書

文書バージョン: 0.1.0  
仮称: Oracle AutoLoop  
対象環境: Windows 11 / PowerShell / Python 3.11以上  
対象リポジトリ: `C:\PROJECT\OracleCouncil`
...
OracleCouncilの開発で現在行っている次の作業を自動化する。
...
- live評価
- 実Claude / 実CodexをOracleCouncil内部から呼ぶ評価
...
自動化システムの状態は、OracleCouncilリポジトリ外へ保存する。

```text
C:\PROJECT\OracleCouncil-automation\
├─ config.json
├─ state.json
├─ runner.lock
├─ prompts\
...
```markdown
<!-- ORACLE_AUTO_RESULT_BEGIN:X-8.20 -->
...
```text
C:\PROJECT\OracleCouncil-automation\runner.lock
```
